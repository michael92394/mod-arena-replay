CREATE TABLE IF NOT EXISTS `character_arena_replays` (
  ...
  `replayQuality` VARCHAR(32) NOT NULL DEFAULT 'legacy_unqualified',
  `qualityReason` VARCHAR(255) NOT NULL DEFAULT 'legacy_or_missing_metadata',
  `packetQualified` TINYINT(1) NOT NULL DEFAULT 0,
  `visibleInBrowser` TINYINT(1) NOT NULL DEFAULT 0,
  `team0PacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `team1PacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `neutralPacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `skippedPacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `team0RealPlayerCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `team1RealPlayerCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `winnerPacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `loserPacketCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `winnerRealPlayerCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `loserRealPlayerCount` INT UNSIGNED NOT NULL DEFAULT 0,
...
-- Replay quality gate metadata. Existing rows default to hidden/unqualified until
-- a future migration or replay recapture proves packet coverage on both teams.
SET @stmt := IF(
